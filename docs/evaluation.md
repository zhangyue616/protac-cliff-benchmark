...
The original primary 2,000 × 77 component-resampling plan and its confidence-interval calculation are not packaged here. The target-plan generator, target mappings, multiplicity matrix, benchmark derivation, feature generation, training code, fitted models, and predictions are also absent. Consequently, the default point-estimate command does not reproduce the primary confidence intervals, and this repository alone is not a complete end-to-end reproduction of the study.
