#!/usr/bin/env python3
"""Replay the fixed primary six-contrast component rescore from saved inputs.

This command reads the frozen eight-model OOF prediction table and the saved
2,000 x 77 component multiplicity plan.  It performs no fit, prediction, draw
generation, download, or structure processing.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Saved primary OOF CSV with all eight model/control streams.",
    )
    parser.add_argument(
        "--draw-plan",
        required=True,
        type=Path,
        help=(
            "Saved 2,000 x 77 plan as NPZ (weights, components) or long CSV "
            "(replicate_id, component_id, draw_count)."
        ),
    )
...
def main() -> int:
    ...
    try:
        ...
        receipt = write_outputs(
            args.output_dir,
            outputs,
            analysis_id="primary_xgboost_vs_random_forest_component_rescore",
            plan=plan,
            component_summary=component_summary,
            input_files={"predictions": predictions_path},
            limitations=(
                "Intervals are conditional on the supplied saved multiplicity plan.",
                "Coverage was not calibrated.",
                "The fixed family was internally locked after same-data early findings.",
                ...
                "This command does not reproduce construction, folds, fitting, or prediction generation.",
            ),
            semantic_checks={
                "truth_values_are_finite_and_nonzero": True,
                "rmse_sqrt_within_seed_before_seed_mean": True,
                "all_77_components_including_empty_retained": True,
